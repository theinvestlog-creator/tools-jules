...
def _calculate_price_ratio(
    ticker1_rows: List[Dict], ticker2_rows: List[Dict]
) -> List[Dict]:
    """Calculates the daily price ratio between two tickers based on their adj_close prices."""
    ticker1_map = {
        row["date"]: Decimal(row["adj_close"])
        for row in ticker1_rows
        if row["adj_close"]
    }
    ticker2_map = {
        row["date"]: Decimal(row["adj_close"])
        for row in ticker2_rows
        if row["adj_close"]
    }
    ...
    for date in common_dates:
        ...
        ratio = (close1 / close2).quantize(
            Decimal("0.000001"), rounding=ROUND_HALF_UP
        )
        series.append({"date": date, "ratio": float(ratio)})

    return series


def _aggregate_to_monthly(daily_series: List[Dict]) -> List[Dict]:
    """Aggregates a daily series to a monthly one, taking the last value of each month."""
    monthly_data = defaultdict(lambda: {"date": "", "ratio": 0.0})

    for point in daily_series:
        month_key = point["date"][:7]  # YYYY-MM
        # Keep overwriting, the last one for the month will stick
...
def run():
    ...
    if not os.path.exists(INDICATOR_REGISTRY_PATH):
        log_structured(
            {
                "level": "error",
                "message": "Indicator registry not found.",
                "path": INDICATOR_REGISTRY_PATH,
            }
        )
        return
    ...
    for indicator in registry.get("items", []):
        ...
        try:
            if indicator_type == "price_ratio":
                if len(tickers) != 2:
                    raise ValueError(
                        f"price_ratio requires exactly 2 tickers, found {len(tickers)}"
                    )
                ...
                if not ticker1_rows or not ticker2_rows:
                    log_data.update(
                        {"status": "skipped", "reason": "Missing ticker data"}
                    )
                    log_structured(log_data)
                    continue

                # --- Calculate daily and monthly series ---
                daily_series = _calculate_price_ratio(ticker1_rows, ticker2_rows)
                ...
                daily_output_path = os.path.join(output_dir, "series.json")
                daily_output_data = {"meta": meta, "series": daily_series}
                atomic_write_text(daily_output_path, json.dumps(daily_output_data, indent=2))

                # --- Write monthly series ---
                monthly_output_path = os.path.join(output_dir, "series_me.json")
                monthly_output_data = {"meta": meta, "series": monthly_series}
                atomic_write_text(monthly_output_path, json.dumps(monthly_output_data, indent=2))
